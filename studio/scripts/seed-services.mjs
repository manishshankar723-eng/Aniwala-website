/**
 * The six disciplines, as they stood before they moved to the CMS.
 *
 * Transcribed wholesale from `config/services.ts` — 734 lines of offerings,
 * pipeline stages, tools and deliverables that needed a developer and a deploy
 * to reword. `related` is kept as slugs here; the migration turns each into a
 * reference once every service document exists, which is why the write has to
 * happen in two passes.
 *
 * FAQs ARE here, and they are the exception to how the site reads them. The
 * live site gets service FAQs from their own `faq` documents, filed against
 * `service:<slug>` — several answers are shared between disciplines and one
 * copy is the only way six pages do not drift. But the MIGRATION has to know
 * them to create those documents in the first place, so the snapshot carries
 * them and `migrateFaqs` reads them from here.
 */
export const SERVICES = [
  {
    ...
    "intro": "The full 3D pipeline, from blockout to a textured asset sitting in your engine. Everything arrives on your naming convention, inside your triangle and texture budgets, and screenshotted in your build before we call it done.",
    "tint": "210 70% 22%",
    "order": 10,
    "offerings": [
      {
  ...
  },
  {
    ...
    "intro": "Visual development that a modeller can actually work from, and finished 2D that ships. We draw to answer questions rather than to decorate a deck — silhouettes, callouts and orthographics, not just a handsome illustration.",
    "tint": "340 55% 24%",
    "order": 20,
    "offerings": [
      {
  ...
  },
  {
    ...
    "intro": "Character, creature and camera work in both 2D and 3D. We approve the performance at blocking, before anything is lit or cleaned up, because a beautiful shot with the wrong acting in it is still the wrong shot.",
    "tint": "28 75% 26%",
    "order": 30,
    "offerings": [
      {
  ...
  },
  {
    ...
    "intro": "Effects that serve the shot rather than announce themselves. Smoke, fire, water and destruction — plus the invisible work, cleanup, roto and integration, that decides whether an audience believes what it's looking at.",
    "tint": "280 50% 26%",
    "order": 40,
    "offerings": [
      {
  ...
  },
  {
    ...
    "intro": "The work between a finished asset and a shipping build. Import, materials, lighting, collision, state machines and the profiling pass that catches what a workstation hides — done by people who read your budgets as requirements rather than suggestions.",
    "tint": "150 45% 20%",
    "order": 50,
    "offerings": [
      {
  ...
  },
  {
    ...
    "intro": "Editorial and finishing for everything that ends up as a video file. Trailers, gameplay captures, explainers and social versions, cut to the beat and delivered in every ratio, duration and codec a platform will ask for.",
    "tint": "195 60% 24%",
    "order": 60,
    "offerings": [
      {
