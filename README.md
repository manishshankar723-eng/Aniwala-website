...
```bash
npm run verify        # dataset + social card + astro check + build + links. What CI runs.
...
npm run check:og      # is the committed social card still painted in the site's colours?
```
...
`scripts/check-og.mjs` compares `scripts/og-source.json` — the palette the
committed social card was actually painted with — against the first `:root`
block of `global.css`, and fails the build when they have drifted apart. The
card is generated by hand and committed, so nothing else would ever notice: a
palette edit would ship a site in one set of colours and a link preview in
another, on every share, indefinitely. It earned that immediately — the
`--color-ink-faint` fix described under *Styling* had no visible connection to
a JPEG anywhere, and this check held the build until the card was repainted.

When it fires, regenerate and commit both files:

```bash
npm i --no-save sharp fontkit
node --env-file=.env scripts/generate-og-image.mjs
npm i                      # puts node_modules back
git add public/og-default.jpg scripts/og-source.json
```

## Accessibility

**Nothing in `npm run verify` checks any of this.** Types, links, the dataset
and the social card are covered; an ARIA role that lies is not. It is caught by
running Lighthouse, or it is not caught at all:

```bash
npm run preview        # serves ./dist — the real build, in another terminal
npx lighthouse http://localhost:4321/ --only-categories=accessibility \
  --preset=desktop --chrome-flags="--headless=new" --view
```

Point it at `npm run preview`, not at the dev server. For accessibility alone
the dev server would do, but nothing else it reports means anything there:
stylesheets are not inlined and nothing is minified, so every performance
number is measuring Vite rather than the site. The `agentic-browsing` category
is worth adding to the run — it shares the accessibility tree audit, so it
moves for the same reasons.

Four rules, one paragraph each, because each of them was broken once and none
of them looked broken.

**An ARIA role is a promise about behaviour, not a description of a layout.**
The booking calendar in `BookCall.astro` carried `role="grid"` over a flat run
of `<button>`s laid out in seven columns by CSS. A grid requires `row` children
containing `gridcell`s, and it tells a screen reader to offer arrow-key
movement across two dimensions, one tab stop for the whole widget and a roving
tabindex — none of which was implemented, so the role advertised an interaction
model that did not exist. It is `role="group"` now: it keeps the accessible
name, which is the only reason a bare `<div>` needed a role at all, and claims
nothing about how to move inside it. Every day button already carries its own
full date. If this ever wants the real grid pattern, the rows, the cells and
the roving tabindex have to ship together — half of it is worse than none.

That one element was also the entire `agentic-browsing` score. That category
has only two weighted audits, and one of them is the accessibility tree.

**An accessible name has to contain the element's visible text.** The brand
link's wordmark is two spans, and they were adjacent with no whitespace between
them, so the link's text content was `ANIWALAStudios`. Nothing shows that on
screen — `.brand-text` is a flex column, so the gap between the two lines is
layout rather than text — and it only exists for something that reads the
element instead of looking at it. The `aria-label` names the link "Aniwala
Studios — home", which does not contain `aniwalastudios`; the space is the
whole difference. The practical cost is voice control: "click Aniwala Studios"
has to hit the thing that visibly says Aniwala Studios. The `{' '}` in
`Header.astro` is load bearing, and it is invisible because a whitespace-only
run between flex items is not rendered. It also means the CMS label and the CMS
wordmark have to stay in agreement — a rebrand that edits one and not the other
puts this back.

**24×24 CSS pixels is the floor for anything clickable.** The header's caret
button measured 23.2px wide: an 11.2px caret with 1.6px of padding on one side
and 10.4px on the other. Eight tenths of a pixel short, which is exactly the
...
audit also fails a target that is big enough but sits too close to the next
one, so growing it the wrong way trades one failure for another.

**Contrast is a question about tokens, not about components.** PageSpeed caught
`--color-ink-faint` in the footer, but the footer was only where it surfaced:
the token was `#61677a` on a `#0b0c10` ground, 3.47:1 against the 4.5:1 that
normal-sized text needs, everywhere it was used. See *Styling* for the values
and the two files that do not inherit the fix.
...
**Never let the CDN invent pixels.** `imageUrl` in `src/lib/sanity/client.ts`
passes `.fit('max')`, which makes a requested width a ceiling rather than an
instruction. Sanity's default fit scales a source *up* to whatever it is asked
for, and every width in this codebase is chosen against a layout rather than
against the file — the header mark is a 100×88 upload requested at 320px, and
the CDN duly returned 320px of blur for 8.9KB when 4.3KB of real detail was all
that existed. `iconUrl`, `webpUrl` and `ogImageUrl` set their own fit and are
deliberately exempt: a social card has to be exactly 1200×630 whatever was
uploaded, and an undersized one is better upscaled than refused by a scraper.
...
### The three inks have a contrast floor

The tokens are not free choices. `--color-ink-faint` is used at 11.52px in the
footer, which is as small as type gets here, and normal-sized text needs 4.5:1
against what is behind it. Both themes were under:

| | was | ratio | now | ratio |
| --- | --- | --- | --- | --- |
| Dark, on `#0b0c10` | `#61677a` | 3.47 | `#828a9f` | **5.66** |
| Light, on `#faf9f5` | `#83879a` | 3.38 | `#6b6f80` | **4.73** |

The dark value also clears AA on `--color-surface` (5.24) and `--color-surface-2`
(4.77), and still sits well below `--color-ink-muted` at 7.46 — "faint" is
still visibly quieter than "muted", so the three-step hierarchy survives the
fix. A tool that measures one theme will never tell you about the other; the
light half was failing silently the whole time.

**Two files do not inherit any of this.** `src/pages/schedule.astro` and
`src/pages/moderate.astro` hardcode their own palette on purpose — they are
rendered out of an email with no CMS, no `global.css` and no design system
behind them, which is the point of them. Their own faint grey was `#6d7285` at
12.8px, 4.09:1, failing the same way and for the same reason. Re-check those
two whenever a text colour moves; nothing links them to the tokens but a
convention.

### The CSS is inside the document

`build.inlineStylesheets: 'always'` in `astro.config.mjs`, and it is the single
biggest thing on this site's critical path.

Measured on a PageSpeed run of the homepage: the document finished at 511ms and
**nothing was painted until 1230ms**. Four `<link rel=stylesheet>` tags were
discovered together at 521ms and three of them landed by 673ms, but
`Blocks.css` — 54KB raw, 8.8KB over the wire — did not arrive until 1196ms,
having lost one h2 connection to two preloaded fonts and a dozen CDN images.
First paint followed it by 34ms. LCP followed first paint by 99ms.

Which means the obvious suspects were all innocent. The curtain in
`Loader.astro`, the font swap and the four-deep GSAP import chain are every one
of them downstream of first paint, and none was the constraint — the page
simply had nothing to paint yet. The scores PageSpeed *shows* are Lantern
simulations; the `observed*` fields in the trace are what actually happened,
and they are where this was visible.

A stylesheet that is in the document cannot lose that race, and here it is not
even a byte trade:

| | before | after |
| --- | --- | --- |
| Homepage document, gzipped | 32.1KB | 47.5KB |
| Separate stylesheet requests | 22.9KB in 4 requests | none |
| **Net first load** | — | **fewer bytes, 4 fewer round trips** |

What it costs, on the record: the CSS is no longer shared between pages, so an
internal navigation carries it again inside the next page's HTML rather than
reading it from cache. That is the right way round for this site. Sessions here
start cold — a portfolio is something people arrive at from a link, look at,
and leave — and an earlier first paint for every one of those visits is worth
more than kilobytes saved on a second click.
...
Both re-encodes are in the bucket. The 16.5 MB original is untouched, and
either can go back in by pasting its URL into the hero's **Background video**
field:

```
video/pieces/new-video-2d49a7833b4b45a1.mp4    <- live now, 96 s, 6.16 MB
video/krazzy-4-hero-loop-720-09d7ed28.mp4      <- the 24 s cut, 1.26 MB
video/krazzy-4-hero-full-720-42d2f622.mp4      <- full reel, 720p, 5.34 MB
```

**The hero has since regressed, and this is what that looks like.** The live
file follows every part of the recipe above except the one that matters: it is
1280×720, silent, `+faststart` correct, keyframed every 0.8 s, and CRF is doing
its job at 512 kbps. It is simply **96 seconds long**, so it is 6.16 MB — 93%
of the homepage's entire weight. Measured against the same source:

| | Size | vs. live |
| --- | --- | --- |
| 96 s (live) | 6.16 MB | — |
| Re-encoded at 30 fps | 5.39 MB | −12% |
| Trimmed to 30 s | 1.87 MB | −70% |
| Trimmed to 20 s | 1.28 MB | −79% |
| Trimmed to 15 s | 911 KB | −85% |

**Duration is the only lever with any leverage.** Halving the frame rate buys
12% and costs a generation of re-encoding, because CRF already spends almost no
bits on the redundant frames. Length is nearly linear. This is why the budget
above is written in megabytes and the recipe's `-t` is 24.

Nothing in the build checks the length of what goes into a Background video
field, and nothing will — a long loop does not fail, it is just slow, which is
the hardest kind of regression to notice from inside the Studio.
